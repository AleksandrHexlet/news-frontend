...
const handleLogout = event => {
  const items = ["menu__user-name", "menu__logout-img", "menu__button_logout"];
  const shoulLogout = items.some(i => event.target.classList.contains(i));
  if (shoulLogout) {
    logOut();
  }
};
...
const createCard = (article, keyword, _id) => {
  ...
  const isMarked = isLogin
    ? getAlreadyMarkedCards(parsedArticle, cards.savedCards)
    : false;
  ...
  newsCard.setHandlers([
    {
      ...
      callback: () => {
        ...
        addOrDeleteCardFromServer(
          data,
          newsCard.renderLoader,
          newsCard.switchMarkButton
        );
      }
    }
  ]);
...
const submitSearchForm = event => {
  if (
    event.target.classList.contains("search-form__button") &&
    searchForm.validate()
  ) {
  ...
  if (event.target.classList.contains("popup__button_signin")) {
    ...
    mainApi
      ...
      .then(results => {
        ...
        localStorage.setItem(LOCAL_STORAGE_TOKEN, results.token);
  ...
  switch (event.type) {
    case "keydown": {
      if (
        ...
        document.querySelector(".popup_is-active")
      ) {
        ClosePopupsEvent();
      }
      break;
