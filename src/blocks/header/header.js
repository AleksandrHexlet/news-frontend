...
export default class Header extends BaseComponent {
  constructor(props, header, menu) {
    ...
    this._handleOpenCloseMobileMenu = this._handleOpenCloseMobileMenu.bind(
      this
    );
  ...
  _handleOpenCloseMobileMenu(event) {
    const hamburger = event.target.classList.contains(
  ...
  switchCloseButton(isActive) {
    const button = this._header.querySelector(".header__button");

    if (isActive) {
      button.classList.add("header__button_close");
    } else {
      button.classList.remove("header__button_close");
    }
  }
  ...
  renderButtons() {
    ...
    if (this._props.isLogin) {
      ...
      this._menuContainer.querySelector(
        ".menu__user-name"
