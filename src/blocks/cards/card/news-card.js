...
export default class NewsCard extends BaseComponent {
  ...
  _setContent(element) {
    ...
    const TEXT_LENGTH =
      window.innerWidth <= WIDTH_FOR_CUT_TEXT
        ? MOBILE_TEXT_LENGTH
        : MAIN_TEXT_LENGTH;

    element.querySelector(".card__text").textContent = this._cutExcessText(
      text,
      TEXT_LENGTH
    );
    element.querySelector(".card__title").textContent = this._cutExcessText(
      title,
      TITLE_LENGTH
    );
    ...
    element.querySelector(".card__pub-time").textContent = this._transformDate(
      date
    );
  ...
  _template(isSavedNews, isLogin, isMarked = false) {
    ...
    <div class="card__control">
      ...
      <a class="card__button-info ${
        !isLogin || isSavedNews ? "card__button-info_is-active" : ""
      }">
        ${
          isSavedNews
    ...
    <p class="card__tag-name ${
      isSavedNews ? "card__tag-name_is-active" : ""
    }"></p>
  ...
  renderLoader() {
    this.cardElement
      .querySelector(".card__loader")
      .classList.toggle("card__loader_is-active");
  }
  ...
  switchMarkButton() {
    this.cardElement
      ...
      .classList.toggle("card__button_mark_is-active");
  }
